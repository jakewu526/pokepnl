...
| CARD-19 | Card whose newest `PriceSnapshot` is older than the stale threshold (14d) | amber "Pricing updated N days ago" line renders under the set line; N matches the real gap |
| CARD-20 | Card with a snapshot inside the threshold | no stale warning rendered at all |
| CARD-21 | Card with no price history (`priceCaptureDate` is null) | no warning, no crash, no `NaN days` or epoch-era day count |
| CARD-22 | Day count derived from the raw snapshot date, not the densified series | matches newest real `PriceSnapshot.capturedDate`; unaffected by interpolated points (see `lib/densify.ts`) |
...
| PORT-31 | `/portfolio` grid tile on mobile (< md) | shows image, name, subtitle, and one per-unit price (same styling as a `CardTile` price) plus a plain-text "More Info" link — no Qty/Cost/Value lines and no Sell/Delete on the tile itself |
| PORT-32 | "More Info" popup (mobile tile) | opens `ItemInfoModal` with Quantity, Cost per unit, Total cost, Market price, Total value, Unrealized $/%, and a "View item page →" link; read-only — no Sell/Delete inside it; closes via the X button, Escape, or a backdrop tap |
| PORT-33 | Same tile at ≥ md | unchanged from before the mobile batch — Qty/Cost/Value/Unrealized lines and Sell/Delete still inline, no "More Info" link |
| PORT-34 | "In your binder" panel on `/cards/[id]` and `/sealed/[id]` | hidden entirely when signed out or when the viewer owns none of that item; when owned, lists every position (one row per condition) with qty/cost/market/value/unrealized, each with its own working Sell and Delete |
| PORT-35 | Same card held in two different conditions | both positions appear as separate rows in the holding panel, each with its own correct per-condition price (condition multiplier applied) and its own Sell/Delete |
| PORT-36 | Sell or Delete from the holding panel | position updates/disappears immediately; `/portfolio`'s grid and table views reflect the change on next load |
...
### Mobile layout & navigation (2026-09-07 mobile batch)
| ID | Case | Expected |
|---|---|---|
| DASH-69 | **Mobile viewport lock** — on every signed-in and signed-out page at 375px wide | `document.documentElement.scrollWidth === document.documentElement.clientWidth`; a phone browser cannot pinch-zoom below 1.0x (nothing to de-center) — this was previously broken by `AuthNav`'s un-wrapped six-link row overflowing the viewport |
| DASH-70 | Pinch-to-zoom **in** on mobile | still works — the fix is an `overflow-x: clip` guard + explicit `viewport` export, not `user-scalable=no` |
| DASH-71 | Top-right avatar circle (mobile, signed in) | replaces the full `Dashboard / My Portfolio / My Transactions / Watchlist / Settings / Log out` row below `md`; links to `/settings`; the full row still renders unchanged at `md` and up |
| DASH-72 | Top-right area (mobile, signed out) | shows the same compact Log in / Sign up pair as desktop — no separate mobile-only signed-out state needed |
| DASH-73 | `/settings` → Account section | shows the signed-in user's name and email plus a Log out button, at every viewport width — this is the *only* logout affordance on mobile now that `AuthNav`'s row is hidden below `md` |
| DASH-74 | `/dashboard` on mobile (< md) | opens directly on the four stat tiles (2×2) + value chart + allocation chart, no hero screen first, and all of it fits one phone viewport (375×812) with no scrolling; the hero and its own timeline mount reappear unchanged at `md`+ |
| DASH-75 | Mobile dashboard's history timeline | dropped below the fold (not in the fitted block); reappears in normal scrolling flow just above Top Movers, `md:hidden` there so it and the desktop mount (inside `DashboardOverview`, `hidden md:block`) never both show |
| DASH-76 | Double-tap the value chart, allocation chart, or (mobile) history timeline | opens that chart full-screen with an X button top-right; Escape and the X both close it; the page itself never zooms during the gesture (`touchAction: manipulation` prevents iOS's native double-tap-zoom from also firing) |
| DASH-77 | Double-tap gating | only active below `md` — a desktop double-click does not open the fullscreen overlay |
| DASH-78 | Allocation chart, zoomed | ring box and legend render at their desktop ("large") size even inside a narrow phone viewport, not the small mobile default |
| DASH-79 | A tap that moves > 10px between down and up (a scroll), or two taps > 300ms apart | does not count as a double-tap; does not open the overlay |
...
| 2026-09-07 | dev | Mobile batch (DASH-69…79, PORT-31…36): `tsc --noEmit`, `eslint .`, and `next build` all clean; server-rendered HTML inspected directly for every changed page to confirm the intended classes/structure landed; interactive flows exercised via a fresh throwaway account (`mobiletest@example.com`, safe to delete from the dev DB) — add-to-collection, the new `HoldingPanel`'s Sell dialog (new `DialogShell` X-button), and the new `ItemInfoModal` "More Info" popup all confirmed working end-to-end | 1 real defect found and fixed pre-ship; full click-driven UI pass still not done this round | Found and fixed a Server→Client boundary violation: `app/dashboard/page.tsx` (a Server Component) was passing a function as `ChartZoom`'s `children` render-prop directly, which React rejects since functions aren't serializable across that boundary — it produced the exact "digest"-only runtime error Next logs server-side, with no console error client-side and no visible symptom beyond a stuck `loading.tsx` fallback. Fixed via a new client-only wrapper, `MobileTimelineZoom`. This session hit the **same "elements measure 0×0, browser pane never composited" limitation logged on 2026-07-31** on unrelated, unmodified routes (e.g. `/watchlist`) even after that fix — confirms it's this test setup's pane, not the app. **Still needs a real click-driven pass** (ideally on an actual phone, per UI-18) before promoting: DASH-74/76/77/78 (dashboard fit and chart double-tap-zoom) and PORT-31/32 (mobile tile + More Info) were verified via DOM/server-HTML inspection only, not a live composited browser. |
